...
def get_fmnist(args):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=args.batch_size, drop_last=True, num_workers=1)
...
class Custom3DShapesDataset(Dataset):
    def __init__(self, data, latents_values):
        self.data = data
        self.latents_values = latents_values
...
def get_3dshapes(args):
    root = os.path.join(args.data_dir+'/3dshapes-dataset/3dshapes.pkl')
    fin = open(root, "rb")
    dataset = pickle.load(fin)
    images = dataset['images']  # array shape [480000,64,64,3], uint8 in range(256)
    labels = dataset['labels']  # array shape [480000,6], float64

    images = images.transpose(0, 3, 1, 2).astype(np.float) / 255.0
    images = images * 2 - 1
    train_kwargs = {'data':images, 'latents_values':labels}
